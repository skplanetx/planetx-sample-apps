i. 개요 - "SNS All Viewer"
	...
	1. 명칭 : SNS 모아보기 (for android)
	
	2. 설명 : Facebook, Google+, Twitter에 등재되는 각종 댓글들을 모아서 볼 수 있는 서비스
              
              3. API 활용 : Social Component API



ii.Sample Project 파일 구성
	
	1. /OpenPlatform_SNS의 AndroidManifest.xml 파일 : Planet X의 라이브러리를 사용할 때 꼭 설정해야 할 

	   permission과 Activity가 선언되어 있습니다.

	2. /OpenPlatform_SNS/libs : 라이브러리 폴더

		- SKPOP_SDK.jar : Planet X server 연동

		- SKPOP_SDK_OAuth.jar : OneID 인증

		- google-play-services.jar : Google+ 연동

		- twitter4j-core-2.1.3-SNAPSHOT.jar : Twitter 연동

	3. /OpenPlatform_SNS/src/com/skp/opx/core/client : 프로젝트 실행에 필요한 OAuth Key, SNS key, 각 API의 
...
iii. 준비 사항
	...
	3. 개발자 센터에 샘플 앱 등록하기
	   ...
	   3) 앱을 등록할 때 [Select Service] 에서 "SK Planet One ID", "Social" 컴포넌트를 꼭 선택해야 합니다.
              
	      이는 앱이 사용하는 API가 속한 서비스별로 사용 승인을 득해야 하기 때문입니다.                   
	 
	4. 샘플 테스트를 위해서는 각 소셜별(Facebook, Twitter, Google+) 사용자 가입이 선행되어야 합니다. 
...
iv. 환경 설정
	...
	3. 다운받은 샘플 프로젝트의 압축을 푼 후 OpenPlatform_SNS 폴더를 eclipse에서 import.

	4. OpenPlatform_SNS/src/com/skp/opx/core/client/Define.java의 OAuth class에 개발자 센터에서 앱 등록 후 발급받은 Client ID, 
	...
	5. 각 SNS 별 앱등록 및 설정
	   ...
	   2) 각 SNS(Facebook, Twitter, Google+)별로 개발자센터 사이트에 앱을 등록하고 인증키를 발급 받습니다.
	   ...
	   4) 발급받은 key 및 ID 정보를 앱 소스에 저장해야 합니다.
		...
		: OpenPlatform_SNS/src/com/skp/opx/core/client/Define.java의 Twitter Key class 및 Facebook Key class에 입력하세요.
