i. 개요 - "Location Tracker"
	
	1. 명칭 : 실시간이동경로알리미
	
	2. 설명 : T map을 활용하여 경로를 검색 및 안내 받을 수 있으며 사용자의 실시간 이동경로를 SMS,네이트온 쪽지로 전송할 수 있는 서비스

	3. API 활용 : NateOn, T map API



ii.Sample Project 파일 구성
	
	1. /OpenPlatform_RPN의 AndroidManifest.xml 파일 : Planet X의 라이브러리를 사용할 때 꼭 설정해야 할 

	   permission과 Activity가 선언되어 있습니다.

	2. /OpenPlatform_RPN/libs : 라이브러리 폴더
	...
	3. /OpenPlatform_RPN/src/com/skp/opx/core/client : 프로젝트 실행에 필요한 OAuth Key, 각 API의 URI 및
...
iii. 준비 사항
	...
	3. 개발자 센터에 샘플 앱 등록하기
	   ...
	   3) 앱을 등록할 때 [Select Service] 에서 "SK Planet One ID", "T map", "네이트온"을 꼭 선택해야 합니다.
...
iv. 환경 설정
	...
	3. 다운받은 샘플 프로젝트의 압축을 푼 후 OpenPlatform_RPN 폴더를 eclipse에서 import.

	4. OpenPlatform_RPN/src/com/skp/opx/core/client/Define.java의 OAuth class에 개발자 센터에서 앱 등록 후 발급받은 Client ID, 
