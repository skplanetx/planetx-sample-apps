i. 개요 - "Melon Info Sharing"
	
	1. 명칭 : 멜론음악공유
	
	2. 설명 : 멜론에서 제공하는 차트를 조회하고 음악정보를 검색할 수 있으며 SNS에 공유할 수 있는 서비스

	3. API 활용 : Melon, Social Component API



ii.Sample Project 파일 구성
	
	1. /OpenPlatform_MSS의 AndroidManifest.xml 파일 : Planet X의 라이브러리를 사용할 때 꼭 설정해야 할 

	   permission과 Activity가 선언되어 있습니다.

	2. /OpenPlatform_MSS/libs : 라이브러리 폴더
	...
	3. /OpenPlatform_MSS/src/com/skp/opx/core/client : 프로젝트 실행에 필요한 OAuth Key, SNS key, 각 API의 
...
iii. 준비 사항
	...
	3. 개발자 센터에 샘플 앱 등록하기
	   ...
	   3) 앱을 등록할 때 [Select Service] 에서 "SK Planet One ID", "멜론", "소셜 컴포넌트"를 꼭 선택해야 합니다.
...
iv. 환경 설정
	...
	3. 다운받은 샘플 프로젝트의 압축을 푼 후 OpenPlatform_MSS 폴더를 eclipse에서 import.

	4. OpenPlatform_MSS/src/com/skp/opx/core/client/Define.java의 OAuth class에 개발자 센터에서 앱 등록 후 발급받은 Client ID, 
	...
	5. 각 SNS 별 앱등록 및 설정
	   ...
	   4) 발급받은 key 및 ID 정보를 앱 소스에 저장해야 합니다.
		...
		: OpenPlatform_MSS/src/com/skp/opx/core/client/Define.java의 Twitter Key class 및 Facebook Key class에 입력하세요.
