//
//  CyworldPhotoAlbumViewController.m
//  All Asset
//
...
#import "CyworldPhotoAlbumViewController.h"
...
@interface CyworldPhotoAlbumViewController ()

@end

@implementation CyworldPhotoAlbumViewController
...
-(void)setupData
{
    ...
    //API 요청
    [self performSelector:@selector(requestFolderList:) withObject:@"me" afterDelay:0.1f];
}
...
/**
 * 설명 : Cyworld 사진첩 게시물 목록 보기
 * RequestURI : https://apis.skplanetx.com/cyworld/minihome/{cyId}/albums/{folderNo}/items
 * Request PathParam : 
 * {cyId} 조회할 대상의 싸이월드 ID입니다. 본인의 싸이월드 ID를 사용할 경우 “me”를 지정하면 됩니다.
 * {folderNo} 폴더의 번호(전체보기의 경우: 0)입니다.
 */
- (void)requestPhotoAlbum:(NSMutableDictionary *)dic
{
    [CommonUtil commonStartActivityCenterIndicator:UIActivityIndicatorViewStyleWhiteLarge];
    
    NSString *cyId      = [dic valueForKey:@"selCyId"];
    NSString *folderNo  = [dic valueForKey:@"selFolderId"];
    
    NSLog(@"cyId : %@, folderNo : %@", cyId, folderNo);
 ...
 * Request PathParam :
 * {cyId} 조회할 대상의 싸이월드 ID입니다. 본인의 싸이월드 ID를 사용할 경우 “me”를 지정하면 됩니다.
 * {folderNo} 저장 폴더의 번호입니다. folderNo는 폴더목록보기 API를 이용하면 알 수 있습니다.
 * {itemSeq} 게시물의 일련번호입니다. itemSeq는 사진첩게시물목록 API를 이용하면 알 수 있습니다.
 */
- (void)requestDetailPhotoAlbum:(NSString *)cyId folderNo:(NSString *)folderNo itemSeq:(NSString *)itemSeq
{
    [CommonUtil commonStartActivityCenterIndicator:UIActivityIndicatorViewStyleWhiteLarge];
    
    NSString *url       = [SERVER_SSL stringByAppendingFormat:CY_PHOTO_DETAIL_PATH,cyId,folderNo,itemSeq];
...
- (void)resultParseAndSave:(NSString *)result
{
    ...
    if ([dic objectForKey:@"folders"] != nil) {
        ...
        [dic setValue:@"me" forKey:@"selCyId"];
        [dic setValue:folderNo forKey:@"selFolderId"];
        [self requestPhotoAlbum:dic];
    //사진첩 게시물 목록 결과 파싱
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (selectedFolderId != nil && [selectedFolderId length] > 0) {
        //[self requestPhotoAlbum:@"me" folderNo:selectedFolderId];
        NSMutableDictionary *dic = [NSMutableDictionary new];
        [dic setValue:@"me" forKey:@"selCyId"];
        [dic setValue:selectedFolderId forKey:@"selFolderId"];
        [self performSelector:@selector(requestPhotoAlbum:) withObject:dic afterDelay:0.1f];
    }
}
